    ...
    toggleFilter(myFilter) is intended to be called whenever a toggle button is pressed, and determines the next function
    to be called
    ...
    toggleDropdown(myFilter) is intended to expand and contract the 3 dropdown menus when toggled
      it handles adjusting visibility of elements contained within the drop down, and adjusting the height of 
      the dropdown card element itself 

    toggleSelection(myFilter) is intended to toggle a filter selection toggle button on and off, and update the search results table 
    accordingly 
      it will update the filter selection toggle button's apperance, and call either toggleVisibilityOn(filter) or 
      toggleVisbilityOff(filter) depending on if a filter selection is being toggled on or off 

    toggleVisbilityOn(filter) and toggleVisbilityOff(filter) are intended to update the search terms used to populate the 
    search results table, and refresh the table 
    ...
    */
      ...
      function toggleSelection(myFilter) {
        ...
        console.log(document.getElementById('resources-table').children[0])
        if (resultCount == 1) {
          console.log(document.getElementById('resources-table').children[0].getAttribute('role'))
      ...
      function toggleVisibilityOff(filter) {
        ...
        //trigger an event to ahve the searchbox update the search results
        searchbox.dispatchEvent(new Event("keyup"))
      }

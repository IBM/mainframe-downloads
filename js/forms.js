...
(function ($, IBM) {
	...
	var me = IBM.namespace(IBM, "common.widget.forms");

	/**
		Published on line form DOM element after line form has been initialized.
	...
	**/
	/**
		Line form fields init.
		<br>Sets up line form text fields and binds them to add/remove class to move the label on focus.

		@method setupLineFormFields
	**/
	me.setupLineFormFields = function () {
		function addCoverClass (inputEl) {
			$(inputEl).closest("p").addClass("ibm-coverfield");
		}

		function removeCoverClass (inputEl) {
			$(inputEl).closest("p").removeClass("ibm-coverfield");
		}

		function addActiveClass (inputEl) {
			$(inputEl).closest("p").addClass("ibm-active");
		}

		function removeActiveClass (inputEl) {
			$(inputEl).closest("p").removeClass("ibm-active");
		}

		function adjustAllTextinputs () {
			$(".ibm-row-form--line").find("input:text, input:password, input[type='search']").each(function () {
				// Add/remove the class on load time if it's pre-filled or not
				if (this.value === "") {
					addCoverClass(this);
				}
				else {
					removeCoverClass(this);
				}
			});
		}
		...
		$(".ibm-row-form--line").find("input:text, input:password, input[type='search']").each(function () {
			...
			// Setup bindings to add/remove class.
			$(input).on("click focus", function () {
				removeCoverClass(input);
		...
		// Fire an event on each line form HTML element to tell subscribers the widget has been initialized.
		$(".ibm-row-form--line").each(function () {
			IBM.common.widget.manager.dispatchInitEvent(this);
		});
	};

	// Onload setup the field labels for the new line form design.
	$(function () {
		if ($(".ibm-row-form--line").length > 0) {
			me.setupLineFormFields();
		}
	});
...
(function($, IBM) {
	...
	/**
		Published on form DOM element after validator has been initialized.
		<br><strong>Example usage</strong>:
		<pre>
	...
	**/
	me.init = function (formEl) {
		...
		**/
		...
		// Do the actual field bindings and setups.
		$form.find("input[type='submit']").on("click.formvalidator", function (evt) {
		...
		// If the form submit button was already hit at least once, we are just checking input validity on the fly,
		//  not only after another submit button press, because this is how the native HTML5 validation works.
		$form.find("input, textarea, select").each(function(){
			$(this).on("keydown.formvalidator paste.formvalidator change.formvalidator", function(){
				var $that = $(this);
		...
		// Destroys the validation bindings. That's all that was done here.
		$form.data("widget-formvalidator", {
			destroy: function () {
				$form.removeClass("ibm-widget-processed").find("input, textarea, select").off(".formvalidator");
				return $form;
			}
		});

		// Add processed state (class) and fire special init event on the HTML element.
...
(function ($, IBM) {
	...
	var me = IBM.namespace(IBM, "common.widget.fileinput"),
		fileInputWidgets = [],
	...
	**/
	...
	/**
		Published on widget's DOM element after the widget has been initialized.
		<br><strong>Example usage</strong>:
		<pre>
	...
	function Fileinput (settings) {
		...
		**/
		...
		function init (elem) {
			try {
				...
				// Merge in translations and add to HTML templates.
				config = $.extend({}, defaults, $elem.data() || {}, settings);
				
				filesControlAddFileHtml = filesControlAddFileHtml.replace('{browse}', config.translationBrowse);
				filesControlRemoveFileHtml = filesControlRemoveFileHtml.replace('{remove}', config.translationRemove);

				// Setup the actual widget.
				setupInitialControls($elem);

				// We wrap the original element inside a container, so we have to find and add 
				//  widget to it after that manipulation.
				$widgetContainer.find("input").data("widget", me);
		...
		/**
			...
			@method Fileinput.adjustInputName
			@private
		...
		**/
		function adjustInputName ($thisEl) {
			...
			if (!elName && $elem.data("multiple")) {
				newName += "[]";
			} 
			else if (elName && $elem.data("multiple")) {
				newName = elName + "[]";
			}
		...
		/**
			...
			@method Fileinput.createNewFileInput
			@private
		**/
		function createNewFileInput () {
			return adjustInputName($elem.clone());
		}
		
		/**
			Takes or creates a file input and wraps it in the ibm-fileinput wrapper with associated add/remove controls.

			@method Fileinput.createNewFileInputContainer
			@private
		...
		**/
		function createNewFileInputContainer ($input) {
			var $newInput = ($input || createNewFileInput()).on("change", doFileChange),
				newEl = $(filesControlWrapperHtml).append($(filesControlAddFileHtml).on("click", doUploadClick)).append($newInput);
		...
		/**
			...
			@method Fileinput.setupInitialControls
			@private
		...
		**/
		...
		/**
			...
			@method Fileinput.adjustFileInputControlLinks
			@private
		**/
		function adjustFileInputControlLinks () {
			...
			if (numFileControls === 0) {
				...
				$widgetContainer.find(".ibm-browsebutton").html("Browse");
			}
		}
		...
		/**
			...
			@method Fileinput.doRemoveClick
			@private
		...
		**/
		...
		/**
			...
			@method Fileinput.doUploadClick
			@private
			@param evt {Event} The click event.
		**/
		function doUploadClick (evt) {
			evt.preventDefault();
			$(evt.currentTarget.parentElement).find("input[type=file]")[0].click();
		}
		...
		**/	
		function doFileChange (evt) {
			var $fileInput = $(evt.currentTarget),
				...
				$removeButton = $(filesControlRemoveFileHtml).on("click", doRemoveClick),
			...
			if ($form.hasClass("ibm-column-form") && fileName.length > 30) {
				fileNameShort = fileName.substring(0, 14) + "..." + fileName.substring((fileName.length - 10), fileName.length);
			}
			else if ($form.hasClass("ibm-row-form") && fileName.length > 15) {
...
/**
	...
	Date picker plug-in pre-processor. 
	<br />
...
(function ($, IBM) {
	...
	var me = IBM.namespace(IBM, "common.widget.datepicker");

	/**
		Init function called on load for any date or time pickers that exist on the page at time of load.
		<br />Inits the pickatime/date plug-in using any user settings to override default that v18 sets.
	
		@method init
		@param el {DOM element} The &lt;input> element you want to add the widget to.
		@param [settings] {Object} Settings to override defaults and element's @data-xxxx attributes.
	**/
	me.init = function (el, settings) {
		var $elem = $(el),
			...
			defaults = {
				...
				localeToUse: IBM.common.util.meta.getPageCc() + IBM.common.util.meta.getPageLc(),
				onClose: function () {
					this.$holder.blur();
				},
 				selectMonths: true,
		...
		// Use es/es for all ES languages.
		if (IBM.common.util.meta.getPageLc() === "es") {
			config.localeToUse = "eses";
		}


		// This is the actual function that inits the widget. We have to get the translations first (ajax promise below), 
		// then call this after translations are retrieved.
		function initWidget () {
			// Init the actual widget and add our standard class to it.
			pickerWidget = $elem.pickadate(config).pickadate("picker");

			$elem.data("widget", pickerWidget);

			// Add common destroy function API.
			// Destroys the widget and puts back the original HTML element that existed before the widget initialized.
			$elem.data("widget").destroy = function () {
				$elem.data("widget").stop();
				$elem.data("widget").$icon.remove();
				$elem.replaceWith($origEl);
				return $origEl;
			};
		
			// Find select lists inside the datepicker widget elment root and prevent/disable 
			//  the select2 widget on select lists inside the date picker.
			pickerWidget.$root.find("select").attr("data-init", false);

			// If they didn't opt-out of showing the icon, add it/overlay it on top of the input field and on 
			//  click of the icon, trigger a click on the input so the datepicker opens.
			if (config.includeicon) {
				$elem.data("widget").$icon = $('<a class="ibm-calendar-link" href="#" tabindex="-1">&nbsp;</a>').insertAfter($elem).on("click", function (evt) {
					evt.preventDefault();
					evt.stopPropagation();
					pickerWidget.open();
				});
		...
		// Request the translations, since defaults are set as EN we don't really care if this fails.
		// If it does fail (silently) it just means the locale xlations didn't exist and will fallback to EN.
		if (config.enabletranslations) {
			$.ajax({
				url: IBM.common.util.config.get("datepickerDataUrl") + config.localeToUse + ".js",
				dataType: "script",
				cache: true
			}).always(function () {
				initWidget();
